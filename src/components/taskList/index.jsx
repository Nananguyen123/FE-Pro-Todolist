...
export default function TaskList(props) {
  ...
  const getTasks =
    props.topic === "done" ? getTasksComplete : getTasksUnComplete;
  const { data, loading, error, page, loadPage, reload } =
  ...
  const inputNewArea = (
    <Form onFinish={handleAddNew} form={form}>
      ...
      <Space>
        ...
        <Button ref={pendingCallAPI} type="primary" htmlType="submit">
          Add
        </Button>
      ...
      <div className="list">
        ...
        <ul className="list-items">
          {loading
            ? Array(10)
                .fill(0)
                .map((item, index) => <Skeleton key={index} active />)
            : data
                ?.sort((task1, task2) => {
        ...
        {isAddNew ? (
          ...
          <button className="add-card-btn btn" onClick={toggleAddNew}>
            Add a card
          </button>
        )}
      </div>
